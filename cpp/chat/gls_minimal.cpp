// CHAT-LS
// Minimal ls -la clone rewritten in C++17 (object-oriented style)
// POSIX.1-2008 compliant, no std::filesystem used.
...
public:
    std::string format(off_t bytes) const {
        ...
        char buf[32];
        if (u == 0) std::snprintf(buf, sizeof(buf), "%.0f%s", size, units[u]);
        else std::snprintf(buf, sizeof(buf), "%.1f%s", size, units[u]);
        return buf;
    }
};
...
public:
    ...
    void print(const FileInfo& info) const {
        ...
        std::snprintf(user, sizeof(user), "%s", pw ? pw->pw_name : "?");
        std::snprintf(group, sizeof(group), "%s", gr ? gr->gr_name : "?");
    ...
    int run(int argc, char* argv[]) {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (S_ISDIR(st.st_mode)) {
                ...
                FileInfo fi{"", argv[i], st, std::nullopt};
                renderer_.print(fi);
            }
        }
        return 0;
...
private:
    void list_directory(const std::string& path, bool show_header) {
        if (show_header) std::cout << path << ":\n";
        auto files = lister_.list(path);
        for (const auto& f : files) renderer_.print(f);
        if (show_header) std::cout << "\n";
    }
