...
#  define HAVE_LONG_OPTS 0
#elif defined(_GNU_SOURCE) || defined(__APPLE__) || defined(__FreeBSD__) || \
      defined(__OpenBSD__) || defined(__NetBSD__) || defined(__DragonFly__) || \
      defined(__linux__)
#  define HAVE_LONG_OPTS 1
#else
#  define HAVE_LONG_OPTS 0
#endif
...
#if HAVE_LONG_OPTS
    G.longs = calloc(num_specs + 1, sizeof(struct option));
    if (!G.longs) return -1;
    for (size_t i = 0; i < num_specs; i++) {
        const opt_spec_t *s = &specs[i];
        G.longs[i].name    = s->long_opt;
        G.longs[i].has_arg = (s->type != OPT_BOOL) ? required_argument : no_argument;
        G.longs[i].flag    = 0;
        G.longs[i].val     = (s->short_opt != '\0') ? s->short_opt : (int)(256 + i);
    }
#endif
...
static int is_help_or_version(const opt_spec_t *s) {
    return s->type == OPT_BOOL && ((s->short_opt == 'h') || (s->short_opt == 'V') ||
        (s->long_opt && (!strcmp(s->long_opt, "help") || !strcmp(s->long_opt, "version"))));
}

int parse_opts(int argc, char **argv) {
    int c, err = 0;
#if HAVE_LONG_OPTS
    while ((c = getopt_long(argc, argv, G.shorts, G.longs, NULL)) != -1)
#else
    while ((c = getopt(argc, argv, G.shorts)) != -1)
#endif
    {
        if (c == '?' || c == ':') return -1;
        int idx = (c >= 256) ? (int)(c - 256) : schema_index_by_short((char)c);
        if (idx < 0) return -1;
...
static void print_range_default(FILE *out, const opt_spec_t *s) {
    if (s->type == OPT_INT && s->min_val <= s->max_val) {
        fprintf(out, " [%lld..%lld]", (long long)s->min_val, (long long)s->max_val);
    } else if (s->type == OPT_FLOAT && s->min_val <= s->max_val) {
        fprintf(out, " [%.6g..%.6g]", s->min_val, s->max_val);
    }
    fprintf(out, " (default: ");
    switch (s->type) {
        case OPT_BOOL:  fprintf(out, "%s", s->default_bool ? "true" : "false"); break;
        case OPT_INT:   fprintf(out, "%lld", s->default_int); break;
        case OPT_FLOAT: fprintf(out, "%.6g", s->default_flt); break;
        case OPT_STRING:fprintf(out, "%s", s->default_str ? s->default_str : "NULL"); break;
    }
    fprintf(out, ")");
}
...
void show_help(FILE *out, const char *progname) {
    ...
    for (size_t i = 0; i < G.n_specs; i++) {
        ...
        if(s->short_opt)
            ...
            fprintf(out, "    ");
#if HAVE_LONG_OPTS
        if(s->long_opt && *s->long_opt) {
            fprintf(out, ", --%s", s->long_opt);
            fprintf(out, "%*s", strlen(s->long_opt)>20 ? (int)0 : (int)(20 - strlen(s->long_opt)), "");
        } else fprintf(out, "%24s", "");
#endif
    ...
    fprintf(out, "\nNotes:\n");
    fprintf(out, "  * Use \"--\" to terminate option parsing before negative operands.\n");
    fprintf(out, "  * When stdin is a pipe and no operands are given, \"-\" is treated as an implicit operand.\n");
